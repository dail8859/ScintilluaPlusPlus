...
private:
	...
	inline sptr_t Call(unsigned int message, T wParam = 0, U lParam = 0) const {
		sptr_t retVal = SendMessage(nppData._nppHandle, message, (uptr_t)wParam, (sptr_t)lParam);
		return retVal;
	}
...
public:
	...
	explicit NotepadPPGateway(NppData nppData) {
		SetNppData(nppData);
	}
	...
	HWND GetCurrentScintillaHwnd() const {
		...
		return (&nppData._scintillaMainHandle)[id];
	}
